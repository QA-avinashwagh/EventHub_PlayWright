...
export class EventBookingComponents {
    ...
    constructor(page: Page) {
        ...
        this.errorFullname = page.getByLabel('Full Name').locator('p');
    ...
    async getPricePerTicket() {
        const pricePerTicket = await this.bkTicketPrice.getByText(/^\$/).textContent();

        if (!pricePerTicket) throw Error("Price per ticket not appeared on the page")
        const price = pricePerTicket.replace(/[^0-9.]/g, '');
    ...
    getBookingRefId(){
       return this.refIdText.textContent();
    }   
